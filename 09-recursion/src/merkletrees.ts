...
async function main() {
  ...
  events.forEach(({ key, size }) => {
    map.set(key, Poseidon.hash([size]));
  });

  logger.info('making first set of proofs');

  logger.info(map.get(Field(8)).toString(), map.tree.height);
  logger.info(map.tree.getNode(1, BigInt(0)));
  for (let i = 0; i < map.tree.height; i++) {
    // logger.info(map.tree.getNode(0, BigInt(0)));
  }
